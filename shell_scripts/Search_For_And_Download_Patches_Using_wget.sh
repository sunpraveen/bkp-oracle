...
# *==================================================================================+
# |
# | FILENAME    : Search_For_And_Download_Patches_Using_wget.sh
# | DESCRIPTION : Given a patch number, search for it in MOS and download it.
# | USAGE       : Search_For_And_Download_Patches_Using_wget.sh <patch_number>
# | AUTHOR      : Praveen B K
# |
# +==================================================================================+
## Variables
...
## Get the patch number as an argument.
export PATCH_NUMBER=$1

## Ensure that the script is called along with one argument parameter.
if [[ $# -ne 1 ]]; then
  printf "\n\nPlease enter the patch number as the argument of the script. Exiting.....\n\n\n"
  exit 2
fi

## Ensure that the patch number so entered is indeed numeric only. If non-numeric, then exit.
if [[ ${PATCH_NUMBER} =~ [^[:digit:]] ]]; then
  printf "The patch number entered - '${PATCH_NUMBER}' - contains non-numeric characters as well. Ensure to enter the patch number as numerals only.\n\n"
  printf "Can't proceed further. Exiting......\n\n\n"
  exit 2
else
  printf "The patch number entered - '${PATCH_NUMBER}' - indeed contains only numerals. Proceeding further.....\n\n"
fi
